...
/**
 * Una sola transacción: kardex, cobro, devolución. En Neon usa un cliente del pool;
 * en el preview usa la transacción de PGLite. Si algo falla, no queda a medias.
 */
export async function withTx<T>(fn: (sql: Sql) => Promise<T>): Promise<T> {
  await getSql();
  if (dbSource === "neon") {
    const pool = globalRef.__pgPool__;
    if (!pool) throw new Error("Pool de Postgres no inicializado");
