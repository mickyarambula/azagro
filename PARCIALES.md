...
|---|---|---|---|---|
| **0** | **Cimientos sin cambiar comportamiento.** (a) El cuadre de § 5 como función pura + prueba sobre datos de hoy (cero en cada partida) + panel junto al cuadre kardex-vs-existencia. (b) ~~Migración aditiva: `qty_returned` **en migración** (hoy solo por `alter` en tiempo de ejecución, `azagro.ts:1643`)~~ **CONSTRUIDO 14-sep-2026** (migración `0032_qty_returned.sql`, commit `553a54a`) — falta la otra mitad: la liga que hoy no existe: qué movimiento de kardex y qué FV pertenecen a **cuál** entrega/recepción (columna nullable en `stock_moves` e `invoices`, sin tocar lo existente — hoy todo es "la única"; verificado hoy 15-sep, sigue sin construirse). (c) ~~Los cinco mensajes "todavía no está construido" (`cancel.ts:117,148,220`; `delivery-reversal.ts:113`; `reversal.ts:136`) pasan a nombrar el paso que ya existe (B2)~~ **CONSTRUIDO 14-sep-2026** (commit `3cb3c57`, mismo día, después de este diagnóstico — verificado hoy: ya no queda ninguna ocurrencia de esa frase en `src/lib/erp/`). | Solo falta: (a) el cuadre § 5, (b) la liga entrega/recepción | Chico (bajó de Mediano) | Lo ya decidido |
...
**Dependencias duras:** 0 → todo; 1 → 2; 3 → 4 y 5; 1 y 3 → 7; ~~A4 antes de 1 y 3~~ y ~~C4 antes de la prueba en producción de cualquiera~~ (§ 6) — **las dos construidas** (A4: commit `b8b6576`; C4: commits `0c16c18`/`262bc95`/`4c8b2ca`/`85effca`), no quedan como dependencia pendiente. Los pasos 0, 1 y 2 se pueden construir ya. Del 3 al 7, cada uno **nombraba** la pregunta que lo gateaba — las ocho ya están contestadas (§ 9, Decisiones 46-54): lo que sigue gateándolos es únicamente el orden de construcción de esta misma tabla, no una decisión pendiente.
