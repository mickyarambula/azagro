...
**Estado (15-sep-2026):** pasos **1-5 construidos** — `migrations/0034_datos_de_prueba.sql`, `scripts/purge-plan.mjs`, `scripts/erp-borrado-pruebas.test.mjs`, `scripts/erp-tablas-clasificadas.test.mjs`, `src/lib/erp/purge.ts` + `scripts/erp-borrado-servidor.test.mjs` (paso 4), y la pantalla en `/importar` + etiquetas en `/bitacora` + `scripts/erp-borrado-pantalla.test.mjs` (paso 5).
...
- **Paso 6:** aún por construir (documentación: `DECISIONES.md`, `PATRONES-DISENO.md`, `ESTADO.md` § 4.15).
...
**Dependencias:** 1 ✓ → 2 ✓ → 3 ✓ → 4 ✓ → 5 ✓ → 6 (pendiente). Los pasos 1-5 construidos y verificados (618/618, `tsc` limpio, y probados en el navegador). Paso 6 aún por hacer. Todo lo hecho es aditivo; ninguna prueba existente cambió.
