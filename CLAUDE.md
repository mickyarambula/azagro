...
| Circuitos de financiamiento: catálogo (paso 0, solo lectura en Ajustes), **etiqueta** `circuit_code` que se guarda/hereda/muestra (paso 1), y **selector** en solicitud/cotización directa/pedido directo — propone según el plazo, solo ADMINISTRADOR lo mueve, candado tras confirmar el pedido, solo Contado y Circuito ASR elegibles (paso 2). **El motor sigue sin leerla** — precio, mora, escalera y reportes en Ajustes hasta el paso 3 | `src/lib/erp/circuits.ts` (`circuitForTerm`, `inheritCircuit`, `circuitLabel`, `SELECTABLE_CIRCUITS`), `src/components/circuit-select.tsx`, migraciones 0024/0025 |
