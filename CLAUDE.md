...
7. UI y copy en español. Sin emojis salvo que lo pidan.
   **Dos versiones de cada documento:** la de pantalla puede explicar todo; la que **sale de la empresa** (papel "Documento", texto de "Enviar", recordatorios) lee **solo** de `src/lib/erp/doc-text.ts` y nunca cuenta cómo funciona el sistema por dentro (Compaq, spread, tasa de costo, Ajustes, bitácora, kardex, umbral, migración, tabla, política, "toca", "pantalla"). Sí explica cómo se calcula lo que se cobra. Expediente al cliente = SOL/COT/PV/FV; al proveedor = SC/OC; nunca cruzados. En papel, lo que falta determinar ("sin TIIE", "sin política") dice "Pendiente de cálculo" y lo que no se cobra ("sin mora") va en "—". **"Pronto pago (est.)" es solo de pantalla**: ni papel, ni documento guardado, ni mensaje (su importe entre cargo y días despeja la tasa de costo, y la bonificación se ofrece cuando conviene). Lo vigila `scripts/erp-documento-limpio.test.mjs`.
