...
test("cableado: el estado de cuenta obedece los interruptores del documento", () => {
  ...
  assert.ok(ec.includes('r.sinPolitica ? "sin política"'), "la columna dice «sin política» en vez de un número inventado (en pantalla; el papel dice «Pendiente de cálculo»)");
});
...
test("cableado: «Sin mora» apaga interés, comisión, FEGA, TIIE y pronto pago", () => {
  ...
  const ec = src("src/routes/statements.tsx");
  assert.ok(ec.includes('r.sinMora ? "sin mora"'), "la columna lo dice en pantalla, no imprime un cero mudo (el papel pone guion: no hay nada que cobrar)");
