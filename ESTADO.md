...
## 7. Construcción del catálogo de circuitos — avance (5-sep-2026)

**Paso 0 (hecho, a4d5020).** Catálogo `credit_circuits` con los cuatro circuitos (Contado, Circuito ASR, Línea Santa Rosa, Línea propia), comisión del ASR copiada de Ajustes; tabla de tasas de dos columnas `funding_rates`, vacía; panel de solo lectura en Ajustes. Nadie lo lee.

**Paso 1 (hecho).** El circuito entra como **etiqueta**: `circuit_code` (nulo permitido, sin selector) en solicitudes, cotizaciones, pedidos y facturas. La migración 0025 asigna lo existente con la regla decidida — plazo 0 → Contado; todo lo demás, corte de Compaq incluido → Circuito ASR; FI, ATC y NC heredan de su documento de origen; facturas de proveedor sin circuito; `route_kind` no se consulta. Desde ahora la etiqueta nace en cada alta y se hereda por la cadena igual que el plazo (`inheritCircuit`): SOL → COT → PV → FV → NC/FI/ATC; la OC del cliente y el pedido manual la sacan del plazo (`circuitForTerm`); el importador la pone en ASR. **Corrección del dueño, mismo día:** la solicitud NUEVA nace SIN circuito (nulo, pantalla "Sin definir", `requestCircuitLabel`) — no en Contado: mientras nadie captura el plazo, el sistema no ha decidido nada, y Contado es justo el circuito que no cobra financiamiento. Se resuelve sola con `inheritCircuit` en cuanto se guarda un plazo, así sea 0. Las solicitudes VIEJAS ya migradas por 0025 (plazo 0 o sin plazo → Contado) no cambian: eso es leer el pasado, no decidir dinero hoy. Cambia con el plazo (solicitud, revisión de cotización, cambio de plazo del pedido, guardar pedido) y queda en bitácora. Se ve de solo lectura en las cuatro pantallas, en cartera y en la explicación del estado de cuenta **en pantalla** (el papel al cliente no la menciona). **El motor no la lee:** `pricing.ts`, `margins.ts`, `credit.ts`, `ladder.ts` y `reports.ts` siguen leyendo Ajustes exactamente igual, y `scripts/erp-circuitos.test.mjs` lo vigila. Verificación contra producción: `scripts/erp-circuitos-verificacion.mjs` (solo lectura; conteo por circuito, etiquetas vs regla, precio implícito vs guardado, instantánea `--guardar`/`--comparar`; salida esperada: cero diferencias).

**Paso 2 (hecho).** Selector de circuito en solicitud, cotización directa y pedido directo (`src/components/circuit-select.tsx`): el sistema propone con la regla del plazo (`circuitForTerm`/`inheritCircuit`) y la persona confirma o cambia — solo Contado y Circuito ASR son elegibles, Línea Santa Rosa y Línea propia salen apagadas "por construir" (`SELECTABLE_CIRCUITS`). Solo ADMINISTRADOR puede moverlo (`assertAdmin` / `member.role === "admin"` en `saveRequestTerms`, `createQuote`, `reviseQuote`, `saveOrder`); cualquier otro rol lo ve, de solo lectura, con el mismo valor en vivo. Candado tras confirmar el pedido (y, para un pedido que vino de cotización, el circuito nunca se elige a mano ahí — se hereda, punto). Cada cambio manual queda en bitácora marcado "(elegido)", con quién y cuándo (como cualquier `writeAudit`). "Tipo de entrega: Circuito ASR" se renombró a "Entrega directa vía ASR" — sigue siendo `route_kind = 'asr'`, la misma lógica de logística, nada más cambió el texto. El motor sigue igual: precio, mora, escalera y reportes en Ajustes, sin tocar (mismo cableado que el paso 1, ahora extendido a `pricing.ts`/`margins.ts`/`credit.ts`/`ladder.ts`/`reports.ts`/`cpo.ts`/`cutover.ts`).
...
**Paso 4 (hecho — construido, pendiente de visto bueno y de subir a main).** Los reportes leen el circuito. (1) `computeDealPnl` usa comisión, base y las DOS tasas congeladas en la factura (`params_snap`) y, si la FV no las tiene, las de la cotización (`quotes.commission_rate` / `cost_rate` / `collection_rate`); el catálogo solo si hace falta; Ajustes nunca. Por ASR los números son idénticos: no se tocó una sola línea del cálculo, solo se AGREGARON campos (`scripts/erp-reportes-circuito.test.mjs` congela la partida y los totales de antes y los compara en +1,000 casos). Lineal: margen 6,951.87 para Azagro, financiamiento de Santa Rosa 4,924.24. (2) **La protección se ve por separado**: por partida y en totales, `lineCost` (costo real de la línea = lo desembolsado × (tasa de costo + spread) × días / 360) y `protection` (lo cobrado al cliente − costo real). DISENO § 5: 4,857.40 + 66.84. Por ASR hay una sola tasa: costo real = costo financiero, protección 0. Sin tasa de costo congelada la protección queda "sin dato", nunca en cero; un total con una partida sin dato queda sin dato. (3) La tarjeta "Costo financiero" del pedido dice la base según el circuito (ASR: costo puesto × (1 + comisión), una sola tasa, protección 0; lineal: la factura a Santa Rosa, con el desglose costo real + protección y sus tasas). (4) `listDealPnl` y `getPanorama` heredan circuito, base, financiamiento de Santa Rosa, costo real y protección; la lista por pedido muestra el circuito bajo el folio y el desglose en la celda de costo financiero del lineal; el Panorama agrega las columnas "Financ. S. Rosa" y "Protección" **solo cuando algún pedido corrió por la línea** (por ASR serían columnas de ceros); el CSV lleva circuito, financiamiento de Santa Rosa, costo real y protección. **NO entra (confirmado con el dueño antes de empezar):** el reparto 50/50 de la mora con Santa Rosa — necesita la factura de Azagro a Santa Rosa y el cobro reportado (Fase 3); la prueba vigila que no se haya construido nada de eso. **Por qué, con las tres razones, para que en la Fase 3 no haya que volver a razonarlo:** (1) la utilidad de la mora es lo que Santa Rosa le cobra al cliente MENOS lo que le cuesta la línea esos días, y lo que Santa Rosa cobra solo se sabe con el cobro reportado (DISENO § 4, pasos 12-13) — no existe todavía; (2) en el lineal la FI la emite Santa Rosa, no Azagro, y el motor de mora (`credit.ts`) ni siquiera lee el circuito hoy; (3) sin la factura de Azagro a Santa Rosa (DISENO § 4, paso 7) no hay contra qué cuadrar el reparto — nada que partir a la mitad. Verificación contra producción: `scripts/erp-circuitos-verificacion.mjs` ganó la sección 5 (núcleo del P&L por pedido con lo congelado: venta, costo, flete, comisión, Capa 1, margen; en el lineal financiamiento de Santa Rosa, costo real y protección) y la mete a la instantánea `--guardar`/`--comparar`; quedan fuera a propósito Capa 2, mora, pronto pago y TC porque cambian con la fecha y los cobros, no con el paso 4. El paso 4 no trae migración.

**Pendiente (en orden):** paso 5 = encender la Línea Santa Rosa en el selector — antes hace falta la Fase 3 del diseño: la FV de Azagro a Santa Rosa, la instrucción de facturación, la factura espejo, el cobro reportado y, con eso, el reparto de la mora. La mora (`credit.ts`) todavía no lee el circuito (Decisión 5 para el lineal: tasa de cobro + spread de mora).
