...
4. ~~**Venta en pesos con costo en dólares: ¿quién absorbe el movimiento del TC entre cotizar y pagar la OC?**~~ **CERRADA el 19-sep-2026, Decisión 90:** lo absorbe **Azagro**, nunca el cliente — la exposición no nace de la venta, nace de haber decidido deberle dólares al proveedor a crédito, y en esa decisión el cliente no participa. Pero se absorbe **cubriéndolo** (pactar el TC con el proveedor, o comprar los dólares al cotizar) o **cobrándolo en el precio a mano**, nunca callado. No va automático en la fórmula: el costo financiero es un hecho, el colchón cambiario es un pronóstico. Construido: el aviso al cotizar (§ 12.11).
...
### 11.6 La exposición cambiaria como cosa propia — **CONSTRUIDA el 18-sep-2026 (A.2, § 12.9)**
...
Los ocho renglones están construidos y probados (`erp-pago-fp-tc`); la reversa los deshace con el mismo signo y no inventa diferencial cuando el abono salió de la cuenta en dólares. Lo que falta alrededor: ~~el contra-movimiento de la reversa no copia `amount_fx`/`fx_rate`~~ (**vencido**: sí los copia desde A.1c, `reversal.ts:423-429`, con `amount_fx` negado — corregido el 19-sep-2026), y `fx_invoiced` no crece para proveedor (la estimación del estado de cuenta abierto sigue sobre `amount_fx` completo, hueco 21).
...
**Los tres avisos, cerrados el 18-sep-2026 (misma tarde, base de pruebas limpia — el dueño ya había borrado los datos, no hubo facturas viejas que corregir):**

(a) **`statements.tsx` y `doc-text.ts`, columna «Ut. cambiaria»:** pantalla Y papel imprimían pesos con signo US$ — el papel tenía el mismo bug (un alias local `money` que en realidad llamaba `moneyIn(n, cur)`), y mi primer cierre solo corrigió la pantalla; el revisor de dinero lo encontró en la segunda pasada. Ahora las dos (`statements.tsx` y `doc-text.ts`, `statementPaperRow`/`statementPaperTotals`) imprimen esa columna siempre en pesos, sin importar si la fila está en el bloque MXN o USD.

(b) **`cpo.ts` (`createCustomerPO`):** el precio de la partida pasa de `z.number().nonnegative()` a `z.number().positive()` — 0 ya no es un precio válido, en ninguna moneda. `cpo.tsx` lo detiene ANTES de mandarlo al servidor, con el mensaje «Falta el precio de \<código\>: escríbelo en \<moneda\>.» — verificado en navegador: sin precio no registra nada; con el precio capturado, registra igual que siempre.

(c) **`circuits-seed.ts` + `circuits.ts` (`saveCircuitCommission`):** el ASR nace con `enabled = false` («Por construir» en Ajustes → Circuitos de financiamiento) — Contado sigue naciendo elegible porque no tiene nada que capturar. En cuanto un administrador captura la comisión de apertura, `saveCircuitCommission` activa el circuito en la misma escritura, y solo si su base ya está construida (`costo_comision`/`costo_margen`) — capturarle una comisión a un circuito «por construir» (Línea propia) no lo activa, porque `circuitTerms` lo sigue deteniendo por la base. Verificado en navegador: empresa nueva → ASR «Por construir» → se captura `0.01` → ASR «Sí», Santa Rosa y Línea propia intactas.

`scripts/erp-precio-en-moneda.test.mjs` (tres pruebas más, incluida la del papel) y `scripts/erp-circuitos-siembra.test.mjs` (ajustada al `enabled = false` de nacimiento); `npm test` **906/906**, `tsc` limpio. **Una aserción de una prueba de esta misma sesión se ajustó** (la fila esperada del ASR en `erp-circuitos-siembra.test.mjs`, que fijaba `true` y ahora fija `false` — es la prueba que este mismo bloque escribió, no una prueba anterior); la copia congelada de `statementPaperRow`/`statementPaperTotals` en `erp-documento-limpio.test.mjs` se actualizó para seguir siendo copia fiel, sin tocar ninguna aserción (13/13 igual). Revisor de dinero, segunda pasada sobre este cierre: **PASA CON AVISOS** — encontró que el papel tenía el mismo bug que la pantalla (corregido arriba); anotó como aviso, no como violación, que `credit_circuits.enabled` no decide dinero en ningún lector (solo la etiqueta de Ajustes) y que una OC del cliente con precio 0 guardada ANTES de este cierre seguiría convirtiéndose sin aviso (no determinado si existen; la base de pruebas está limpia). **Tercera pasada (ya con el papel corregido): PASA CON AVISOS, sin hallazgos nuevos** — verificado en navegador (cliente Contado en USD, cobro en pesos a un TC distinto del pactado): pantalla y papel muestran «−$5,000.00», nunca «US$». Anotó un aviso menor, pre-existente y fuera de este cierre: `getLiveStatement` (`ops.ts`) calcula `dueNow` sumando `liveFx` (pesos) contra `saldo`/`liveMora` ya divididos, y nadie lo lee hoy (ni pantalla ni papel ni prueba fuera de su propia definición) — si algún día se imprime, saldría mal en dólares.

### 12.9 Construido — A.2, la posición cambiaria (Decisiones 83-86), 18-sep-2026

**Sin migración.** La pasada de solo lectura (nueve lectores en paralelo) verificó columna por columna que toda la materia prima ya existía después de A.1a, A.1b, A.1c y L4a, y que el patrón de calcularlo en vivo sin columna ya estaba construido y probado en `credit-limit.ts` (Decisión 51). Lo único que no existía era una fecha de vencimiento para la orden de compra — y ahí no se inventa una: va a su propia cubeta «Sin fecha» (Decisión 83).

**Motor puro** `src/lib/erp/fx-position.ts` (sin un solo import, como `parciales.ts`, para que la prueba lo cargue directo y fije los números): `bucketFor`/`bucketize` (ocho cubetas), `coverageByBucket` (dinámica 1), `classifyDeal` (dinámica 2), `netPosition`, `sensitivity`, `revaluation`. La regla del tipo de cambio («un TC real es > 1») no se duplica: vive en `isUsdFx` y se aplica en la frontera — la consulta entrega `fxAgreed` en null cuando no hay TC pactado, y el motor nunca vuelve a decidir qué es un tipo de cambio.

**Consulta** `src/lib/erp/fx-position-query.ts` (`getFxPosition`), en vivo y sin columna:
- Facturas vivas en dólares de los dos lados, `residual ÷ fx_agreed`, con `inv_class = 'product'` para dejar fuera la mora (FI) y el ajuste por TC (ATC), que **nacen en pesos** aunque vengan de una factura en dólares.
- Lo comprometido de venta: `so.total − Σ FV vivas` (los dos en pesos desde L4a) ÷ `so.fx_rate`, con `state in ('confirmed','done')` porque en bodega propia entregar no factura (D47), y **sin** el filtro de contado de `creditExposure` (esa regla es de línea de crédito, no de moneda).
- Lo comprometido de compra: `po.total − Σ FP nacidas`, **las dos ya en dólares** (`po.total` vive en la moneda de la orden, al revés que el pedido; las FP aportan su `amount_fx`). Nunca por cantidad recibida: una orden directa jamás se recibe y contarla por cantidad la contaría dos veces.
- Dólares en caja por cuenta con `usdCashAverage`, la misma regla pura que lee Bancos.

**Pantalla** `/posicion`, vecina de Vencimientos y Bancos, con módulo propio en `pathModule` (`banks`) — sin ese renglón, `pathModule` la habría mandado a «dashboard» y la habría abierto cualquier rol. La sensibilidad y la revaluación exigen además ver márgenes. **Tarjeta en el inicio** («Exposición neta USD · dólar de hoy · lo que se mueve por cada peso»), del mismo lugar que la pantalla: nunca una segunda fórmula que diga otro número.

**Lo que se cerró de paso:** `fxToday()` en fx.ts, el único lugar donde se resuelve «el TC de hoy» (había catorce, y uno contestaba distinto: tomaba el último renglón sin tope de fecha, así que el TC del lunes se congelaba en un pedido del viernes); Ajustes rechaza capturar un TC ≤ 1 (se veía guardado y detenía en silencio cotizar, recibir y facturar); un pedido en dólares exige un TC real (`isUsdFx`), no solo mayor que cero; **Decisión 86**, un gasto pagado desde una cuenta en dólares declara su TC y guarda `amount_fx`/`fx_rate` (antes un gasto de $5,000 sacaba 5,000 DÓLARES y destruía el costo a promedio móvil del resto); y el aviso menor que dejó L4a: `dueNow` ya no divide `liveFx`, que es pesos por naturaleza.

`scripts/erp-posicion-cambiaria.test.mjs` (26, con los números y el signo fijados); `npm test` **933/933**, `tsc` limpio, ninguna prueba existente cambió salvo el literal de `dueNow` en `erp-precio-en-moneda.test.mjs` — la prueba que el propio L4a escribió ese mismo día, actualizada con la corrección.

**Verificado en navegador (PGlite fresco):** OC de 10 × 1,000 USD a 18.5 recibida → FP-0001 de USD 10,000; compra de 6,000 USD a 18.40; COT → PV → FV de 8 × 1,000 USD a 18.5. La pantalla dijo: nos deben **US$8,000**, debemos **US$10,000**, caja **US$6,000** (TC promedio 18.4 · $110,400), **posición neta US$4,000**, **$4,000 por cada peso** que se mueva el dólar, y **$600** de revaluación (los 6,000 dólares comprados a 18.40 valen 18.50 hoy; los documentos en cero porque su pactado ES el dólar de hoy). Cubetas: «Hoy» US$8,000 por cobrar; «31-60 días» US$10,000 por pagar, **US$6,000 cubiertos con los dólares en caja y US$4,000 abiertos**. Gasto de $5,000 desde la cuenta en dólares: el TC se propone en 18.5 y el campo dice «Saldrían US$270.27»; registrado, la cuenta baja **US$270.27** y no 5,000 dólares; borrando el TC se detiene con su aviso. Devolución de 2 sobre la FV: «Nos deben» pasa de US$8,000 a **US$6,000** y «Venta comprometida» se queda en `—` (sin pedido fantasma). Ajustes rechaza guardar un TC de 1: la tabla se queda con su único renglón.

**Dos errores que atrapó la verificación en navegador** (no las pruebas): lo comprometido de una orden salía en **US$540.54 en vez de US$10,000** (la orden guarda su total en dólares y yo lo estaba dividiendo otra vez entre el TC), y un pedido surtido de inventario — cuya orden no lleva `so_id` — salía como **«cubierto, dinámica 2»** sin tener ninguna compra ligada. Los dos quedaron con prueba propia.

**Revisor de dinero: NO PASA a la primera.** Dos violaciones de la misma familia, las dos en «lo comprometido de venta», las dos ya resueltas años atrás en `creditExposure` y no copiadas aquí: la consulta sumaba **todas** las facturas del cliente del pedido que no estuvieran revertidas, así que la **NC espejo de una entrega revertida** y la **NC de una devolución** entraban como facturación NEGATIVA y **doblaban** lo comprometido — US$16,000 en vez de US$8,000 tras revertir una entrega de un pedido de 8 × 1,000 USD, y US$2,000 de pedido fantasma tras devolver 2. Cerrado copiando la regla que ya existía: `name like 'FV-%' and reverses_id is null`. Cerrados también: lo **cerrado corto** ya no aparta exposición para siempre (Decisión 62, la misma resta de `credit-limit.ts`); un pedido **sin fecha de pago convenida** ya no hereda la fecha de captura (mandaba un contado a «Vencido con 8 días» — Decisión 83 dice que no se inventa una fecha); un documento **sin TC pactado** ya no entra a `netoUsd` ni a la sensibilidad (la pantalla decía «no entran en ningún total en pesos» y el renglón de abajo los multiplicaba: ahora es verdad, Decisión 85); el **promedio de dólares en caja** se lee con el mismo orden y el mismo campo que Bancos (antes por fecha aquí y por id allá: un cobro retrofechado daba dos promedios distintos para la misma cuenta); `dueNow` ya no suma pesos a una fila que sale en dólares; **un traslado entre dos cuentas en dólares arrastra el costo** en vez de perderlo (es el mismo dinero cambiado de cuenta: salía al promedio y entraba «sin TC», y la revaluación perdía $100 por cada 1,000 dólares movidos sin que un dólar saliera de la empresa) — eso completa la Decisión 86, que ya lo prometía.

**Un hallazgo del navegador, fuera de A.2 pero corregido aquí:** `/sales/nuevo` (un pedido levantado sin cotización) nace en dólares y proponía la primera partida con el **precio de lista en pesos** — 18,500 bajo una columna que dice «PRECIO / UOM (USD)» —; las partidas que se agregan después sí convertían. Era el agujero de L4a todavía abierto en un solo renglón: si el vendedor aceptaba el número propuesto, el pedido nacía **18.5 veces más grande**. Ahora la primera partida pasa por `salePriceShown` como las demás, con prueba en `erp-precio-en-moneda` (verificado en navegador: TOTAL USD 1,000.00 donde antes decía USD 18,500.00).

**Avisos anotados, no tocados:** el gasto pagado desde la cuenta en dólares sale al promedio móvil sin registrar el diferencial contra el TC declarado (en el ejemplo, $27.03); el `fx_result` de la FP sigue fuera del P&L del pedido (transversal (i), § 13.4); `getUpcomingPayable` cuenta los ATC de proveedor mientras `getUpcomingDue` los filtra; y una FV en dólares nacida antes de L4a solo se distingue por su fecha.

**Lo que NO se puede construir de § 11.6, y por qué:** la cobertura de la **dinámica 1 por deal**. La compra de dólares nace sin `partner_id`, sin `so_id` y sin `po_id`, y los dólares en caja son un pozo común a promedio móvil sin lotes: no existe forma honesta de decir «estos dólares son los que compré para aquella orden». Por eso se deriva a nivel empresa y por cubeta (Decisión 84, decisión del dueño). La dinámica 2 sí se deriva entera, por deal.

**Avisos que quedan anotados** (encontrados en la pasada de solo lectura, fuera del alcance de A.2): una transferencia USD→USD entre dos cuentas en dólares sigue saliendo al promedio y entrando «sin TC»; una FV en dólares nacida ANTES de L4a es indetectable desde su propia fila (solo por la fecha); `getUpcomingPayable` cuenta los ATC de proveedor mientras `getUpcomingDue` los filtra; y el diferencial de la FP sigue fuera del P&L del pedido.

### 12.10 Construido — el diferencial de la compra dentro de la utilidad (Decisiones 87-89), 18-sep-2026

**La transversal (i) de § 13.4, cerrada.** La utilidad del pedido leía un solo diferencial —el del cobro al cliente— y la compra en dólares pagada con pesos dejaba el suyo escrito en la factura del proveedor, con el signo correcto, **sin que lo leyera nadie**: `fx_result` del lado proveedor no aparecía en una sola pantalla del sistema. Ahora la fórmula lleva los dos, cada uno con su nombre: «Diferencial cambiario (cobro al cliente)» y «(pago al proveedor)». Es lo que ya decía § 11.7 — «los dos diferenciales realizados».

**Un solo lugar donde se pregunta por la compra de un pedido** (`src/lib/erp/deal-supplier.ts`): el recorrido `pedido → purchase_orders.so_id → po.name → invoices.origin` estaba copiado a mano en cinco lugares, cada uno con un filtro de estado distinto. Una sexta copia con un sexto criterio era el riesgo real del bloque.

**Campo propio, nunca dentro de `fxIncome`.** El Panorama agrega `fxIncome` por razón social del **cliente**: sumarle ahí el diferencial del proveedor habría hecho aparecer una pérdida de compra como «Dif. TC» de un cliente que no tuvo nada que ver, sin un solo cambio visible en esa pantalla. Y es del **pedido entero**, como los gastos y la mora: entra una vez por `order` en `mergeDealPnl`, y la utilidad por factura queda byte a byte.

**El caso huérfano, dicho con palabras (Decisión 88).** `so_id` solo lo escribe la aceptación de una cotización. Dos negocios idénticos surtidos de la misma compra —uno con su orden ligada, otro de inventario— darían números distintos. El pedido sin compra ligada enseña por qué, y manda a la posición cambiaria, que sí lo ve a nivel empresa. Es el mismo criterio de la Decisión 84.

**Dos hallazgos de la pasada de solo lectura, más graves que el problema original** (dos de cada cuatro investigaciones, como dice la metodología):

1. **Agujero de dinero (Decisión 89).** «Recibir todo lo pendiente» tenía un camino propio que llamaba a `bornSupplierDebt` —una factura por ORDEN, idempotente por folio de orden—. Con una recepción parcial previa, esa guarda encontraba la factura del primer evento, devolvía null, y **el resto de la mercancía entraba al kardex sin cuenta por pagar**, contra la Decisión 14. Un parche de septiembre (ir por partidas solo si había cierre corto) tapaba una esquina del mismo agujero. Ahora hay **un solo camino**: toda recepción va por partidas, mina su folio `RCP` y hace nacer su factura por lo que entró en ESE evento, así que Σ(facturas de una orden) = lo recibido, siempre.
2. **Veinticuatro columnas del esquema vivían solo en el código.** Existían porque algún camino corría `alter table … add column if not exists` antes de usarlas, y no estaban en ninguna migración — entre ellas `purchase_orders.so_id`, la única liga entre una compra y su pedido, y `fulfill_kind`, que decide si una orden se recibe en bodega. Peor: `computeDealPnl` corría **siete** de esas sentencias por llamada y el Panorama la llama hasta 500 veces por carga — tres mil quinientas sentencias de esquema para dibujar una tabla. Migración **0045**, declarativa (`if not exists`, no cambia nada donde ya existían), más los dos índices que faltaban y sí se caminan en bucle: `po_so_idx` e `inv_origin_idx`.

**De paso:** el ajuste por TC **a favor de Azagro** nace con saldo negativo («POR COBRAR AL PROVEEDOR») y el filtro `residual > 0.009` lo dejaba fuera de todos los totales — dinero a favor que no se veía en ninguna pantalla. `getUpcomingPayable` ahora usa `abs(residual)`.

`scripts/erp-diferencial-compra.test.mjs` (13, **cuatro de ellas con números de verdad**: −5,000 de diferencial bajan la utilidad exactamente 5,000, +5,000 la suben, con dos facturas de venta entra una sola vez, y sin compra ligada la utilidad queda byte a byte); `npm test` **947/947**, `tsc` limpio. Pruebas existentes actualizadas: trece, todas porque congelaban un mecanismo que se quitó, ninguna porque cambiara una regla.

**Revisor de dinero: PASA CON AVISOS.** Ninguna violación de las diez reglas — sin doble conteo (el spread mide otro tramo y sigue fuera de la fórmula, el ajuste por TC se excluye simétrico al lado cliente, y con N facturas de venta el término entra una sola vez), el signo verificado con la cuenta completa en los dos sentidos, la migración aditiva y comprobada ejecutándose contra una base nueva, y **cero cambios en `pricing.ts`, `margins.ts`, `ladder.ts`, `credit.ts`, `stock.ts`, `cost.ts`, `doc-text.ts`, `credit-limit.ts` y `circuits.ts`**. Cerrados antes de commitear los cinco que sí eran defectos:
- **`bornSupplierDebt` se quedó sin un solo llamador** y tres documentos —`CLAUDE.md`, el comentario del código y la propia Decisión 89— decían que la usaba brokeraje. No era cierto: brokeraje usa la de por evento, dentro de `deliverPartial`. **Se borró**: dejar exportada una función muerta cuya guarda era justamente la que abría el agujero es dejar la trampa puesta para el siguiente. Cinco pruebas apuntaban a ella y ahora apuntan a `bornSupplierDebtByReceipt`, que lleva las mismas reglas (Decisión 30, se detiene sin plazo; Decisión 36, una revertida no cuenta).
- **El Panorama dejó de cuadrar por columnas:** la utilidad ya llevaba el término y ninguna columna visible lo explicaba, así que la fila de un cliente bajaba 5,000 pesos por una compra que no era suya. Ahora son **dos columnas con nombre**, «Dif. TC cobro» y «Dif. TC pago», y lo mismo en el Excel de utilidad por pedido.
- **`abs(residual)` se había arreglado en un lector y no en el otro:** las alertas y el correo seguían dejando fuera el ajuste a favor de Azagro.
- **Los dos filtros de `deal-supplier.ts` no decían lo mismo** sobre una orden cancelada.
- **El término entró sin una sola prueba que lo midiera con números** — las ocho originales fijaban frases, y «una prueba que cambie el orden de las palabras rompe; una que cambie el signo, no». Ahora hay cuatro con números.

**Aviso que queda y es decisión del dueño:** el mismo hecho económico da dos utilidades distintas según el botón que se apretó al pagar. Con tratamiento «utilidad» el diferencial va a la factura y ahora entra al pedido; con «ajuste» nace un documento aparte y no entra. La exclusión es **correcta y simétrica** con el lado cliente (el ajuste es cartera, no utilidad), pero la asimetría entre los dos botones es real y conviene decidirla.

**Lo que NO va en este bloque:** el spread cambiario (transversal ii — depende de 8.2-4, abierta), el ajuste por TC de proveedor dentro de la utilidad, la columna del Panorama, `getCompanyPnl`, el gasto en dólares de la Decisión 86, y consolidar las cinco copias viejas del recorrido pedido → compra → factura en la función única.

### 12.11 Construido — el aviso al cotizar en pesos con costo en dólares (Decisión 90), 19-sep-2026

**Quién absorbe ya está decidido: Azagro, nunca el cliente.** La exposición no nace de la venta —el cliente compró un precio en pesos— sino de haber decidido deberle dólares al proveedor a crédito, una decisión en la que el cliente no participa. Lo que faltaba era que quien cotiza lo VIERA antes de comprometer el precio.

`src/lib/erp/fx-exposure.ts` (puro, sin imports): `quoteFxExposure` recupera los dólares dividiendo el costo en pesos entre el TC con el que llegó ahí —nunca se reconvierte con el del día— y `utilityAfterFxMove` dice en cuánto queda la utilidad si el dólar se mueve. **Con la venta en dólares no dice nada**: ahí el riesgo viaja con el precio y es del cliente (Decisión 2). Un costo marcado USD sin TC usable tampoco se convierte a la fuerza.

**Aquí no se pronostica nada, y es el punto.** No hay «dólar esperado» ni colchón sugerido: el costo financiero se mete solo en el precio porque hay tasa y hay días; dónde va a estar el dólar en un mes no lo sabe nadie. Meterlo a la fórmula construiría el precio y el margen reportado sobre una adivinanza, y encarecería también las cotizaciones que sí se iban a cubrir, donde el colchón es puro sobreprecio. `pricing.ts` ni se entera. El colchón, si se quiere, lo pone la persona subiendo el margen o el TC — los dos campos ya existían.

**Un hueco que había que cerrar antes:** la lista a proveedores armada desde la solicitud **nacía siempre en pesos** y no había forma de capturar un precio en dólares por ese camino — el único que lo permitía era la cotización a proveedores levantada aparte. Con eso, el aviso casi nunca podía saltar donde de verdad se cotiza. Ahora la solicitud pregunta «Les pides precio en» MXN/USD, y la moneda se congela en cuanto alguien captura un precio (cambiarla después movería costos ya convertidos).

`scripts/erp-exposicion-al-cotizar.test.mjs` (10, con los números del caso del dueño); `npm test` **957/957**, `tsc` limpio, ninguna prueba existente cambió.

**Verificado en navegador (PGlite fresco), con los números exactos del caso:** lista a proveedores en USD, AARFS cotiza **US$1,000** la pieza, dólar de la tabla **17.50** → costo puesto **$17,500.00** («proveedor en USD × TC 17.5»); cotización al cliente en **pesos** con 20 % → precio **$21,875.00**, utilidad **$4,375.00 / LTS · total $43,750.00 (20.0 %)**, importe **$218,750.00**. Y debajo: «El costo de esta cotización es en dólares: USD 10,000.00 al tipo de cambio 17.5 — y le estás cotizando en pesos. Por cada peso que se mueva el dólar antes de pagarle al proveedor, tu utilidad se mueve **$10,000.00** — de $43,750.00 a **$33,750.00** si sube un peso». Cambiando la moneda de la cotización a USD el aviso **desaparece**. Y con el precio del proveedor ya capturado, el selector de moneda de la lista se bloquea con su razón.

**Lo que sigue de este tema, y NO va aquí:** el total del mes —cuánto costó en pesos el movimiento del dólar sobre las compras—, que es la continuación de § 12.10. Automatizar el colchón solo cuando ese número exista y salga del historial del propio Azagro.

### 12.12 Dos cierres cortos encontrados en la pasada de la medición mensual, 19-sep-2026

**La pared de privacidad, rota en el estado de cuenta.** `getLiveStatement` elige socios con `is_customer = true`, pero sus **cuatro** consultas de facturas traían todas las del socio **sin mirar `kind`**. Un socio marcado a la vez cliente y proveedor metía sus facturas de COMPRA —con su `fx_result` y la columna impresa «Ut. cambiaria»— al papel que se le manda al cliente. Es la regla 5, no un detalle de reporte, y el barrido de palabras de `erp-documento-limpio` no lo atrapaba: sus prohibidas no incluyen «proveedor». Cerrado en las cuatro, con prueba que las fija una por una.

**La tercera tabla que no cuadraba.** La Decisión 87 metió el diferencial del pago al proveedor dentro de «Utilidad final»; el Panorama y el Excel ganaron su columna, pero la tabla «por pedido» de `/reportes` se quedó sin ella — la utilidad bajaba y nada visible lo explicaba. Ahora lleva «Dif. TC pago», como las otras dos.
...
Las dos que habían quedado para decidir, resueltas por criterio con aval del dueño.

**(a) El diferencial entra al «Resultado», pero solo lo absorbido.** El P&L del periodo se llamaba «Resultado» y no incluía un peso de lo que costó el dólar — mal en la única dirección que importa, porque esconde pérdidas. Ahora lleva el término, con **tarjeta propia** para que la cuenta cuadre con lo que está encima. Pero entra **solo lo absorbido**: al pagar se elige entre dejar la diferencia como utilidad/pérdida o convertirla en un ajuste por TC, y lo segundo es una cuenta viva —por cobrar o por pagar—, no una pérdida. **Es la regla que el Excel del dueño ya fijaba del lado cliente** («lo que se convirtió en documento ATC es cartera, no utilidad», en `dealPnlCore`), aplicada igual del lado proveedor. El número sale de `fxCostOfPeriod`, **el mismo que lee la pantalla**: nunca dos cuentas del mismo hecho.

El corte necesita el `kind` porque el ajuste nace con `amount = −fxDiff` mientras que el resultado derivado lleva el signo de `fxResultDeltaFor`, que solo invierte del lado proveedor: `enAjuste = Σ(cliente ? −amount : amount)`, `absorbido = total − enAjuste`. Con el caso documentado —factura pactada a 19.00 cobrada a 18.60, tratamiento «ajuste»— el resultado es −4,000 y el ajuste +4,000: **absorbido 0**, porque no hubo pérdida, hubo $4,000 por cobrar. Con tratamiento «utilidad» no nace ajuste y entra completo.

**(b) Las fechas de la nota de crédito y del contra-pago se quedan como están.** Se habían señalado como defecto de periodo: una devolución de octubre sobre una venta de septiembre baja las ventas de octubre. **No es un defecto:** una nota de crédito y una reversa son hechos del día en que se hacen, y recalcularlos hacia atrás reabriría un mes ya cerrado — además de chocar con la Decisión 16, «nada se borra». Lo que sí estaba mal era el **conteo**: «N facturas producto» sumaba las notas de crédito como si fueran ventas, así que tres «facturas» podían ser dos ventas y una devolución. El importe las sigue sumando (una devolución resta, que es correcto); el conteo ahora **excluye el prefijo `NC-`** — y no incluye solo el `FV-` a propósito: **las facturas del corte Compaq traen su propio folio**, así que filtrar por `FV-` las habría dejado de contar sin que nadie lo pidiera.

`npm test` **982/982**, `tsc` limpio, ninguna prueba existente cambió.

**Revisor de dinero: NO PASA a la primera.** El hallazgo era del tamaño del bloque: **revertir un cobro movía hacia atrás el Resultado de un mes ya cerrado.** El ajuste se contaba filtrando por `state <> 'reversed'`, así que al revertirlo —meses después— dejaba de contar en SU mes y ese mes, reimpreso, cambiaba $4,000; mientras el contra-pago caía en el mes nuevo sin nada que lo compensara, moviéndolo otros $4,000. Dos meses movidos por un hecho que en neto no fue ni ganancia ni pérdida, y exactamente lo que la propia Decisión 92 dice que no debe pasar. **Cerrado contando el ajuste el día que NACE y descontándolo el día que se REVIERTE** (`cancelled_at`, que `reversal.ts` sí escribe): cada mes cierra en cero por su cuenta, igual que se autocancela el par de pagos.

Cerrados también: la tarjeta «Diferencial cambiario» se dibujaba por el total y no por lo absorbido, así que un mes donde las dos mitades se cancelaran al centavo movía el Resultado sin renglón que lo explicara; el **inicio**, que enseña la misma utilidad neta, no tenía el renglón; y un comentario de pantalla seguía diciendo que el término «queda fuera del Resultado», lo contrario de lo que hace el código.

**Y la crítica más útil:** las pruebas que había escrito **fijaban texto, no números** — una de ellas llegaba a comprobar su propia aritmética (`-4000 - -4000 === 0`) sin ejecutar una línea del código. Por eso la aritmética se sacó a **`splitFxCost`, puro y probado con números** en los cuatro casos, los dos lados mezclados, la caja, y el mes de la reversa.
...
### 13.8 Lo que sigue después de A.2

Con A.1a, A.1b, A.1c, L4a y A.2 cerrados, la cadena entera declara moneda y tipo de cambio, y la exposición que queda se ve. Lo que sigue **no es captura, es lectura del resultado**, y son las dos transversales de § 13 más lo que el dueño decida:
